<?php
	...
	$plugin_path = trailingslashit( '../wp-content' . substr( $path_to_wp[1], 0 ) );	
?>
var shortcode_generator_url = "<?php echo esc_url( TEMPLATE_DIRECTORY_URI ); ?>/inc/shortcodes/admin/",
	SelectedShortcodeType,
	tb_dialog_helper = {
		SelectedShortcodeType: '',
		needsPreview: false,
		...
		setupShortcodeType: function ( shortcode ) {
			SelectedShortcodeType = shortcode;
			this.SelectedShortcodeType = shortcode;
		},

		loadShortcodeDetails: function() {
			if (SelectedShortcodeType) {
				var a = this;
				// Clean out the table rows before applying the new ones.
				jQuery( '#options-table' ).html( '' );
				jQuery.getScript(shortcode_generator_url + "js/theme-" + SelectedShortcodeType + ".js", function () {
					a.initializeDialog();
		...
		initializeDialog: function (){
			if (typeof frameworkShortcodeAtts == "undefined") {
				jQuery("#shortcode-options").append("<p>Error loading details for shortcode: " + SelectedShortcodeType + "</p>");
			} else {
...
jQuery(document).ready( function($) {
	tb_position = function() {
		...
		if ( tbWindow.size() ) {
			...
			var tbContent = tbWindow.find('#TB_ajaxContent'),
				cW = tbWindow.width(),
				cH = tbWindow.height(),
				cW = ( 720 < cW ) ? 720 : cW;
				
			tbContent.width( cW - 30 ).height( cH - 48 );
		}
	};
